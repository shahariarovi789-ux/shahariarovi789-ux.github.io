        ...
        {CATEGORIES.map((cat) => {
          ...
          return (
            <button
              ...
              className={cn(
                "relative inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs font-mono font-semibold uppercase tracking-wider transition-all",
                isActive
                  ? "text-white"
            ...
            >
              {isActive && (
                <motion.span
                  ...
                  className="absolute inset-0 rounded-full bg-blue-600 shadow-[0_0_15px_rgba(59,130,246,0.4)]"
                  transition={{ type: "spring", stiffness: 400, damping: 30 }}
                />
              )}
              <span className="relative z-10">{cat}</span>
              <span className={cn("relative z-10 text-[10px] opacity-60", isActive && "opacity-90")}>
                ({count})
              </span>
            </button>
          );
